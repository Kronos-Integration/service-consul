		...
		_start() {
			return consul.agent.service.register(this.consulDefinition()).then(f => {
				...
				consul.status.leader().then(leader => {
					this.info(level => `Consul raft leader is ${JSON.stringify(leader)}`);
				});

				consul.status.peers().then(peers => {
					this.info(level => `Consul raft peers are ${JSON.stringify(peers)}`);
				});

				this.kronosNodes().then(nodes => {
					this.info(level => `Kronos nodes are ${JSON.stringify(nodes)}`);
				});
